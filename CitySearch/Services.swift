...
protocol OperationService {
    var result: ((_ response: [CitiesDataItem], _ filterDict: [Character: [CitiesDataItem]]) -> Void)? { get set }
    init(result: @escaping (_ response: [CitiesDataItem], _ filterDict: [Character: [CitiesDataItem]]) -> Void, completionBlock: @escaping () -> Void)
}

class ServiceRead: Operation, OperationService {

    var result: ((_ response: [CitiesDataItem], _ filterDict: [Character: [CitiesDataItem]]) -> Void)?
    ...
    required init(result: @escaping (_ response: [CitiesDataItem], _ filterDict: [Character: [CitiesDataItem]]) -> Void, completionBlock: @escaping () -> Void) {
        self.result = result
...
class ServiceSearch: Operation {
    ...
    var text: String
    var cities: [Character: [CitiesDataItem]]

    init(text: String, cities: [Character: [CitiesDataItem]], result: @escaping (_ response: [CitiesDataItem]) -> Void, completionBlock: @escaping () -> Void) {
        self.result = result
        self.text = text
        self.cities = cities
